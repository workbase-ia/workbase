...
export default function MinhaRede() {
  ...
  try {
    const response = await fetch(`${API_URL}/conexoes/ignorar/${conviteId}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}` }
    });
  ...
  <div className="max-w-7xl mx-auto p-6 bg-slate-50 min-h-screen">
    ...
    <div className="lg:col-span-1 space-y-4">
      ...
      {convites.length === 0 ? (
        <div className="p-4 text-center text-gray-500 text-sm bg-white rounded-lg border border-dashed border-gray-300">
    ...
    <div className="lg:col-span-3">
      <form onSubmit={handleSearchSubmit} className="bg-white p-4 rounded-lg border border-gray-200 mb-8 flex flex-col md:flex-row gap-4 items-end md:items-center">
        <div className="flex gap-2 w-full md:w-auto flex-1">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
            <input type="text" placeholder="Nome, cargo ou habilidade..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" value={termo} onChange={(e) => setTermo(e.target.value)} />
          </div>
        ...
        <div className="flex gap-4 w-full md:w-auto">
          <div className="relative w-1/2 md:w-40">
            <select className="w-full p-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:border-blue-500 text-gray-700 appearance-none" value={filtroArea} onChange={(e) => setFiltroArea(e.target.value)}>
          ...
          <div className="relative w-1/2 md:w-28">
            <input type="text" placeholder="UF" className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" value={filtroEstado} onChange={(e) => setFiltroEstado(e.target.value)} />
          </div>
        </div>
      </form>
