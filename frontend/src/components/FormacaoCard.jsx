...
export default function FormacaoCard({ formacao, MeuPerfil, onProfileUpdate }) {
  const { token } = useAuth();
  
  const [isEditing, setIsEditing] = useState(false);
  const [tempList, setTempList] = useState(formacao || []);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState(null);

  const isEmpty = !formacao || formacao.length === 0;

  // Sincroniza o estado temporário se a prop 'formacao' mudar
  useEffect(() => {
